...
public class MazeDisplay extends Application {
	...
	public void start(Stage stage) {
		...
		mazeControl = new MazeController(NUM_ROWS, NUM_COLUMNS, this);
	...
	private HBox setupControlButtons(){
		...
		stepButton.setOnAction(value ->  {
			this.mazeControl.doOneStep(MILLISECOND_DELAY);
		});
	...
	private Group setupMaze(){
		...
		for(int i = 0; i< NUM_ROWS; i++){
			for(int j =0; j < NUM_COLUMNS; j++){
				...
				rect.setFill(color[mazeControl.getCellState(new Point(i,j))]);
				mirrorMaze[i][j] = rect;
	...
	public void redraw(){
		for(int i = 0; i< mirrorMaze.length; i++){
			for(int j =0; j < mirrorMaze[i].length; j++){
				mirrorMaze[i][j].setFill(color[mazeControl.getCellState(new Point(i,j))]);
			}
		}
	}
	...
	public void step(double elapsedTime){
		if(!paused) {
			mazeControl.doOneStep(elapsedTime);
		}
	}
